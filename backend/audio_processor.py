...
def _refine_lag(query_y, ref_y, sr, coarse_lag_samples, radius_seconds=0.3, step_samples=128):
    ...
    # match_ratio: 归一化相关系数，范围 [0, 1]
    match_ratio = abs(best_score) / best_n if best_n > 0 else 0.0
    return int(best_lag), float(best_score), match_ratio
...
def align_audio(
    ...
    if fine_tune:
        fine_lag_samples, score, match_ratio = _refine_lag(
